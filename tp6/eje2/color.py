class Color:
    ...
    def variarAzul(self, valor:int):
        ...
        if self.__azul + valor > 255:
            ...
            self.__azul += valor
    def establecerColor(self, valorRojo:int, valorVerde:int, valorAzul:int):
        """Establece el Color, en los valorres recibidos por parametro"""
        if not isinstance(valorRojo,int) or valorRojo < 0 or valorRojo > 255:
            raise ValueError("Debe ser un numero entre 0 y 255")
        
        if not isinstance(valorVerde,int) or valorVerde < 0 or valorVerde > 255:
            raise ValueError("Debe ser un numero entre 0 y 255")
        
        if not isinstance(valorAzul,int) or valorAzul < 0 or valorAzul > 255:
            raise ValueError("Debe ser un numero entre 0 y 255")
        
        self.establecerRojo(valorRojo)
        self.establecerVerde(valorVerde)
        self.establecerAzul(valorAzul)
